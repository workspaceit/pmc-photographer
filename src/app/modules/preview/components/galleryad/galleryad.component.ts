...
@Component({
  selector: 'app-galleryad',
  templateUrl: './galleryad.component.html',
  styleUrls: ['./galleryad.component.css'],
  providers:[AdvertisementService]
})
export class GalleryadComponent implements OnInit,AfterViewInit {
...
constructor(private rout: ActivatedRoute,private advertisementService: AdvertisementService) {
  ...
  if(galleryIdStr!=null){
    this.galleryId = Number( galleryIdStr );
  }
  if(popUpIdStr!=null){
    this.popUpAdId = Number( popUpIdStr );
  }
}
...
public fetchGalleryAdvertisement() {
  ...
  this.advertisementService.getById(this.galleryId)
    .subscribe(result=>{
      this.advertisementConfig.gallery.isEndOfAd = true;
      this.advertisements.push(result);
      this.rotationGalleryAd().then(()=>console.log("ROTATION CALLED"));
    },error=>{
...
/*  this.advertisementService.getByEventIdAndType(this.pmcadId,
...
public fetchPopUpAdvertisement() {
  ...
  this.advertisementService.getById(this.popUpAdId).subscribe((data)=>{
